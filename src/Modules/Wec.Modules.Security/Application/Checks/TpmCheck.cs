...
internal sealed class TpmCheck : ISecurityCheck
{
    ...
    {
        // Win32_Tpm requires elevation on most systems; access denied becomes a
        // requires-elevation execution outcome, never an informational finding.
        ...
        if (tpm.IsFailure)
        {
            return CheckFindings.NotRun(CheckId, tpm.Error!);
        }
