...
describe('recent compare clients', () => {
  ...
  it('treats malformed or unsupported cached views as a miss', () => {
    ...
    localStorage.setItem('wec.view.client-compare-recent', JSON.stringify({ version: 1, hosts: 'PC-A' }));
    expect(loadRecentCompareHosts()).toEqual([]);
  });

  it('moves a completed pair to the front, de-duplicates client identities and keeps eight hosts', () => {
