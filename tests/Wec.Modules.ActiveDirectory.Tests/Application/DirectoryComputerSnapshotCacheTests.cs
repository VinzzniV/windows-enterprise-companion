...
public sealed class DirectoryComputerSnapshotCacheTests
{
    private readonly IClock _clock = Substitute.For<IClock>();
    private readonly DirectoryComputerIdentityQuery _query = new(new("example.test", null, ScanCredentials.CurrentUser),
        "example.test", Guid.NewGuid());

    public DirectoryComputerSnapshotCacheTests() => _clock.UtcNow.Returns(DateTimeOffset.UtcNow);
    private DirectoryComputerSnapshotCache Create(int limit = 64) => new(_clock,
        Options.Create(new ActiveDirectoryOptions { IdentityCacheMaximumEntries = limit }));
    private Task<Result<DirectoryComputerIdentityResult>> Load(CancellationToken _) =>
    ...
    public async Task FailedAttemptKeepsSameSourceFactsUntilOriginalRetention()
    {
        ...
        await cache.LoadAsync(_query, _ => Task.FromResult(Result.Failure<DirectoryComputerIdentityResult>(new(ErrorCode.AccessDenied, "Denied"))), CancellationToken.None);
    ...
    public async Task CredentialChangeClearsEvidenceAndDiscardsLateCompletion()
    {
        using var cache = Create();
        await cache.LoadAsync(_query, Load, CancellationToken.None);
        var completion = new TaskCompletionSource<Result<DirectoryComputerIdentityResult>>(TaskCreationOptions.RunContinuationsAsynchronously);
    ...
    public async Task SameQueryRefreshCoalescesAndCacheReadDoesNotWaitForSource()
    {
        ...
        var completion = new TaskCompletionSource<Result<DirectoryComputerIdentityResult>>(TaskCreationOptions.RunContinuationsAsynchronously);
        int calls = 0;
        Task<Result<DirectoryComputerIdentityResult>> Read(CancellationToken _) { calls++; return completion.Task; }
